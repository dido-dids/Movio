package cz.muni.fi.pv256.movio.uco373993.sync;
...
public class UpdaterSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        List<Movie> saved = movieDao.fetchAll();
        for (Movie movie : saved) {
            try {
                Movie m = mMDBApi.loadMovie(movie.getId(), TheMovieDBApi.API_KEY).execute().body();
                if (movie.equals(m)) {
                    movieDao.createOrUpdate(m);
                    updateNotification(m.getId(), m.getTitle());
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private void updateNotification(Long id, String title) {
    ...
    private void initRetrofit() {
        ...
        mMDBApi = retrofit.create(TheMovieDBApi.class);
    }
}